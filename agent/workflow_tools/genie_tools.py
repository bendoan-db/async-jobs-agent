"""
Databricks Genie tool for the LangGraph agent.

This tool wraps the GenieAgent from databricks_langchain to allow
...
def create_genie_tool(
    ...
    Args:
        ...
        description: Description of what this Genie space can answer.
                    If not provided, uses the space's default description.
    ...
    # Create the GenieAgent instance
    ...
    def query_genie(query: str) -> str:
        ...
        Args:
            query: The natural language query about the data.

        Returns:
            The Genie's response with data insights.
        """
        try:
            logger.info("Querying Genie space %s: %s", space_id, query[:100])

            # GenieAgent expects a list of messages in chat format
            messages: list[dict[str, Any]] = [{"role": "user", "content": query}]
            response = genie_agent.invoke(messages)
            ...
            logger.error("Error querying Genie space %s: %s", space_id, e, exc_info=True)
